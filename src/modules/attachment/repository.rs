use super::model::{Attachment, AttachmentListResponse, AttachmentResponse, NewAttachment, UpdateAttachmentRequest};
...
pub async fn find_by_user(
    db: &DBSqlite,
    user_id: String,
) -> anyhow::Result<AttachmentListResponse> {
    ...
    let attachments: Vec<AttachmentResponse> = results.into_iter().map(Into::into).collect();
...
pub async fn insert(db: &DBSqlite, new_attachment: NewAttachment) -> anyhow::Result<Attachment> {
